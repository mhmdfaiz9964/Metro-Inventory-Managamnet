                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-{{ $cheque->status == 'paid' ? 'success' : ($cheque->status == 'pending' ? 'warning text-dark' : 'danger') }}">
                            ...
                            <td class="text-center">
                                ...
                                @if($cheque->status == 'rejected')
                                    <span class="text-danger">This cheque has been rejected/returned</span>
                                @endif
                                ...
                                <div class="modal fade" id="returnChequeModal{{ $cheque->id }}" tabindex="-1" aria-labelledby="returnChequeLabel{{ $cheque->id }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <form action="{{ route('admin.cheques.return.store', $cheque->id) }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="cheque_bank" value="{{ $cheque->bank_id ?? '' }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const paidBankWrapper = document.getElementById('paidBankWrapper');
    const paidBankSelect = document.getElementById('paidBankAccount');

    paymentMethod.addEventListener('change', function() {
        if(this.value === 'fund_transfer') {
            paidBankWrapper.classList.remove('d-none');
            paidBankSelect.setAttribute('required', true);
        } else {
            paidBankWrapper.classList.add('d-none');
            paidBankSelect.removeAttribute('required');
        }
    });
